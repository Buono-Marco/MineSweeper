...
public class MineField extends JPanel {
	...
	private void mark(int row, int col) {
		// if you visit a square at (row,col) and
		// if bombCount(row,col) is zero, then you
		// should also automatically visit all the
		// neighbors of that square
		state[row][col] = STATE_VISITED;
		if(bombCount(row,col) == 0) {
			if (row > 0) {
				if (col > 0 && !(state[row-1][col-1] == STATE_VISITED) && !mined[row-1][col-1])
					visit(row-1, col-1);
				if (!(state[row-1][col] == STATE_VISITED) && !mined[row-1][col])
					visit(row-1, col);
				if (col < COLS-1 && !(state[row-1][col+1] == STATE_VISITED) && !mined[row-1][col+1])
					visit(row-1, col+1);
			}
			if (col > 0 && !(state[row][col-1] == STATE_VISITED) && !mined[row][col-1])
				visit(row, col-1);
			if (col < COLS-1 && !(state[row][col+1] == STATE_VISITED) && !mined[row][col+1])
				visit(row, col+1);
			if (row < ROWS-1) {
				if (col > 0 && !(state[row+1][col-1] == STATE_VISITED) && !mined[row+1][col-1])
					visit(row+1, col-1);
